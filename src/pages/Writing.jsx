import './Writing.css'

function Writing() {
  return (
    <div className="app">
      <div className="container">
        <section className="writing">
          <h1 className="page-title">Writing</h1>
          <div className="page-content">

          <h2>Criticism</h2>
            <p>
                <strong>DEATH TO SLOP</strong>, <a href="https://mayhathaway.substack.com/p/death-to-slop">Substack</a> (2025)<br/>
              <strong><i>Sight & Sound</i> 2022</strong>, <a href="https://mayhathaway.substack.com/p/sight-and-sound-2022">Substack</a> (2022)<br/>
            </p>
            <br/>

          <h2>Fiction</h2>
            <p>
              <strong>How to Wear White to a Wedding</strong>, <a href="https://okaydonkeymag.com/2021/02/05/how-to-wear-white-to-a-wedding-by-may-hathaway/">Okay Donkey Magazine</a> (2021)<br/>
              <strong>Eden, Idaho</strong>, <a href="https://www.havehashad.com/hadposts/eden-idaho">HAD</a> (2021)
            </p>
            <br/>
            
            <h2>Creative Nonfiction</h2>
            <p>
              <strong>Mother Tongue</strong>, <a href="https://sinetheta.net/17.html">Sine Theta Magazine</a> (2020)<br/>
              <strong>Meritocracy</strong>, <a href="https://vagabondcitylit.com/2020/12/14/meritocracy-by-may-hathaway/">Vagabond City Lit</a> (2020)<br/>
            </p>
            <br/>

            <h2>Honors</h2>
            <p>
              Scholastic Art & Writing Awards: National Gold Medal, Best-in-Grade in Personal Essay/Memoir (2021)<br/>
              National YoungArts Honorable Mention Winner in Creative Nonfiction (2021)<br/>
              Brooklyn Public Library's 2021 Teen Writing Contest: First Prize Winner in High School Prose (2021)<br/>
              Scholastic Art & Writing Awards: National Gold Medal in Personal Essay/Memoir (2020)
            </p>
            <br/>

          </div>
        </section>
      </div>
    </div>
  )
}

export default Writing